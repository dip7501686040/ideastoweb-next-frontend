      ...
      >
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            ...
            {/* Products — visible to tenant admin only when "product" service is enabled */}
            {isTenantAdmin && hasService("product") && (
              <Link
                href="/products"
                className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-200 ${
                  pathname === "/products" ? "bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg" : "text-gray-300 hover:bg-gray-800 hover:text-white"
                }`}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* Bookings — tenant admin when booking service is enabled */}
            {isTenantAdmin && hasService("booking") && (
              <Link
                href="/bookings"
                className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-200 ${
                  pathname === "/bookings" ? "bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg" : "text-gray-300 hover:bg-gray-800 hover:text-white"
                }`}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {isTenantAdmin && (
              <Link
                href="/settings"
                className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-200 ${
                  pathname === "/settings" ? "bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg" : "text-gray-300 hover:bg-gray-800 hover:text-white"
                }`}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
            ...
            {/* Services — master admin only */}
            {isMasterAdmin && (
              <Link
                href="/services"
                className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-200 ${
                  pathname === "/services" ? "bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg" : "text-gray-300 hover:bg-gray-800 hover:text-white"
                }`}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                ...
                <div className="flex-1 flex items-center justify-between">
                  ...
                  <span className="px-1.5 py-0.5 bg-purple-500 text-white text-xs rounded font-semibold">M</span>
                </div>
              </Link>
            )}
            ...
            <div className="flex items-center">
              ...
              <div>
                <h1 className="text-xl font-semibold text-gray-900">
                  {visibleAccessControlItems.find((item) => item.href === pathname)?.name ||
                    navigation.find((item) => item.href === pathname)?.name ||
                    (pathname === "/products" ? "Products" : pathname === "/bookings" ? "Bookings" : pathname === "/services" ? "Services" : pathname === "/settings" ? "Settings" : "Admin Portal")}
                </h1>
