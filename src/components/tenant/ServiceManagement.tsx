...
export default function ServiceManagement({ tenantCode }: ServiceManagementProps) {
  ...
  const loadData = async () => {
    try {
      ...
      const isRbacEnabled = enabled.some((s) => s.code === "rbac")
      if (isRbacEnabled) {
        try {
          const creds = await rbacApi.getSuperAdminCreds(tenantCode)
          setSuperAdminCreds(creds)
        } catch {
          setSuperAdminCreds(null)
        }
      } else {
        setSuperAdminCreds(null)
      }
                  ...
                  <div className="flex items-start flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-2 space-y-1.5">
                        ...
                        {!["auth", "user", "product", "rbac"].includes(enabled.code) && <div className="text-xs text-gray-500">Service endpoints will be available after implementation</div>}
                        {enabled.code === "rbac" && (
                          <>
                            <div className="flex items-center text-xs">
                              <span className="text-gray-600 w-16">Admin:</span>
                              <a
                                href={`http://admin.${tenantCode}.${process.env.NEXT_PUBLIC_MAIN_DOMAIN || "localhost"}${process.env.NODE_ENV === "production" ? "" : ":3000"}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-blue-600 hover:text-blue-700 underline font-mono"
                              >
                              ...
                              <div className="mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                                ...
                                <div className="space-y-1.5">
                                  <div className="flex items-center gap-2">
                                    <span className="text-xs text-yellow-700 w-16 flex-shrink-0">Email:</span>
                                    <span className="text-xs font-mono text-gray-800 flex-1 break-all">{superAdminCreds.credentials.email}</span>
                                    <button
                                      onClick={() => {
                                        navigator.clipboard.writeText(superAdminCreds.credentials.email)
                                        setCopiedField("email")
                                        setTimeout(() => setCopiedField(null), 2000)
                                      }}
                                      className="text-xs text-yellow-700 hover:text-yellow-900 flex-shrink-0 px-1.5 py-0.5 rounded hover:bg-yellow-100 transition-colors"
                                      title="Copy email"
                                    >
                                  ...
                                  <div className="flex items-center gap-2">
                                    <span className="text-xs text-yellow-700 w-16 flex-shrink-0">Password:</span>
                                    <span className="text-xs font-mono text-gray-800 flex-1 break-all">{showPassword ? superAdminCreds.credentials.password : "••••••••••••"}</span>
                                    <button onClick={() => setShowPassword((v) => !v)} className="text-xs text-yellow-700 hover:text-yellow-900 flex-shrink-0 px-1.5 py-0.5 rounded hover:bg-yellow-100 transition-colors">
                                      {showPassword ? "Hide" : "Show"}
                                    </button>
                                    <button
                                      onClick={() => {
                                        navigator.clipboard.writeText(superAdminCreds.credentials.password)
